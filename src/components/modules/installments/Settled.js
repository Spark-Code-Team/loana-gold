...
const Settled = () => {
    ...
    useEffect(()=>{
        const loadInstallment = async () => {
            ...
            if (!error) {
              if (installmentStatus === 'unpaid') {
                setUnpaidInstallment([response.data]);
              } else{
                setPaidInstallment(response.data);
              }
            }
          };
        loadInstallment()
    ...
    return(
        ...
        ">
            ...
            {installmentStatus !== 'paid' && unpaidInstallment>0 && (
                unpaidInstallment.map((p, index) => (
            ...
            {installmentStatus === 'paid' && Array.isArray(paidInstallment) && (
                paidInstallment.map((p, index) => (
                    <div key={index} className="...">
                        <div className="
                        w-[848px]
                        h-full
